...
class Top2Vec:
    ...
    ######################### mod methods #########################
    def _get_document_vector(self, corpus):
        '''
        return an array (size(corpus), vector size) of this corpus based on pretrained embedding model
        '''
        (corpus_token_embeddings,
         corpus_tokens,
         corpus_labels) = contextual_token_embeddings(corpus,
                                                      batch_size=32,
                                                      model_max_length=512,
                                                      embedding_model="sentence-transformers/all-MiniLM-L6-v2")
        avg_embeddings, chunk_tokens = sliding_window_average(corpus_token_embeddings,
                                                              corpus_tokens,
                                                              window_size=50,
                                                              stride=40)
    ...
    def get_theta(self,test_corpus):
        ...
        '''
        assert self.contextual_top2vec, "Contextual Top2Vec model is required for this method"
        assert self.c_top2vec_smoothing_window is not None, "not init ctop2vec yet"
        ...
        doc_dist = np.array(doc_dist)
        doc_top_tokens = dict()
        doc_top_token_dists = dict()
        doc_topic_distribution = np.zeros((len(test_corpus), len(topic_vectors)))
        doc_topic_scores = np.zeros((len(test_corpus), len(topic_vectors)))
        document_labels = np.array(test_corpus_labels)
        for doc_ind in tqdm(np.unique(document_labels), desc="Calculating document topic distributions"):
            # Get indices and relevant data for the current document
            doc_inds = np.where(document_labels == doc_ind)[0]
            token_topics = doc_top[doc_inds]
